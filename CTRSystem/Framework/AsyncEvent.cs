...
namespace CTRSystem.Framework
{
	// Source: http://stackoverflow.com/questions/12451609/how-to-await-raising-an-eventhandler-event
	public class AsyncEvent<TEventArgs> where TEventArgs : EventArgs
	{
		...
		public async Task InvokeAsync(object sender, TEventArgs eventArgs)
		{
			...
			await Task.WhenAll(tmpInvocationList.Select(c => c(sender, eventArgs)));
		}
	}
}
